...
struct MusicDatabaseTests {
    ...
    func contributorCreditRemoval() async throws {
        ...
        try await database.updateTrackContributorCredit(trackCredit.contributor.id, in: track.id, role: trackCredit.role, position: trackCredit.position, creditedName: "Guest Track", newRole: .composer)
        #expect(try await database.albumContributors(albumID: album.id).first?.creditedName == "Guest Album")
        let updatedTrackCredit = try #require(await database.trackContributors(trackID: track.id).first)
        #expect(updatedTrackCredit.creditedName == "Guest Track")
        #expect(updatedTrackCredit.role == .composer)
        ...
        try await database.deleteTrackContributor(updatedTrackCredit.contributor.id, from: track.id, role: updatedTrackCredit.role, position: updatedTrackCredit.position)
    ...
    func catalogueContent() async throws {
        ...
        #expect(try await database.albums(creditedTo: contributor.id).map(\.id) == [album.id])
    }
    ...
    @Test("Completed available rescans require confirmation before marking an asset missing")
    func rescanMissingAssetReview() async throws {
        let database = try MusicDatabase(url: temporaryDatabaseURL())
        try await database.migrate()
        let root = try await database.createStorageRoot(.init(displayName: "Music", lastKnownPath: "/Music", bookmarkData: Data([1])))
        ...
        let review = try #require(await database.missingAssetReviews(batchID: rescan.id).first)
        #expect(review.id == assetID)
        #expect(try await database.digitalAssetIDs(albumID: albumID) == [assetID])

        try await database.confirmAssetMissing(assetID, in: rescan.id)
    ...
    func playlists() async throws {
        ...
        let originalItems = try await database.playlistItems(playlistID: playlist.id)
        #expect(originalItems.map(\.trackID) == [second.id, first.id])
        try await database.movePlaylistItem(originalItems[1].id, to: 1)
        #expect(try await database.playlistItems(playlistID: playlist.id).map(\.trackID) == [first.id, second.id])
        let movedItems = try await database.playlistItems(playlistID: playlist.id)
        try await database.removePlaylistItem(movedItems[1].id)
        #expect(try await database.playlistItems(playlistID: playlist.id).map(\.trackID) == [first.id])
        try await database.renamePlaylist(playlist.id, to: "Road trip")
