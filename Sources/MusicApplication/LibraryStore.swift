...
public final class LibraryStore: ObservableObject {
    ...
    public func updateAlbumContributorCredit(_ credit: ContributorCredit, in albumID: AlbumID, creditedName: String?) async throws { guard let database else { throw DatabaseError.notFound("Catalogue database") }; try await database.updateAlbumContributorCredit(credit.contributor.id, in: albumID, role: credit.role, position: credit.position, creditedName: creditedName); try await reload() }
    public func updateTrackContributorCredit(_ credit: ContributorCredit, in trackID: TrackID, creditedName: String?) async throws { guard let database else { throw DatabaseError.notFound("Catalogue database") }; try await database.updateTrackContributorCredit(credit.contributor.id, in: trackID, role: credit.role, position: credit.position, creditedName: creditedName); try await reload() }
