...
9. The first preloading package exposed a Forward-button failure in real use. The player now treats a prepared player as optional: if it cannot start, it immediately retries the same file through the ordinary open path instead of leaving playback stuck.
