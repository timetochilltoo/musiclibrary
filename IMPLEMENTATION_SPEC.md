...
- Thirty-four automated tests across four test suites, last verified with a rebuilt `swift test` followed by `swift test --skip-build` on 22 July 2026.
...
- A packaged iPad app composition root and automatic manifest-date polling.
...
The next coding slice is **iPad application composition and refresh monitoring**. Do not allow companion clients to alter the catalogue.
