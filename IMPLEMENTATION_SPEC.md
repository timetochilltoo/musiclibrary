...
- Device-local iPad favourites and recent-play history complete: `CompanionPreferenceStore` persists starred published album IDs and the newest 20 locally played album IDs alongside companion state. Album detail can add/remove a favourite, the album list displays stars, filters favourites, and shows Recently Played after a local playback start. Snapshot refreshes do not alter those preferences. They never enter the snapshot or Mac catalogue.
