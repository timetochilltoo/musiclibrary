...
- Thirty-three automated tests across four test suites, last verified with a rebuilt `swift test` followed by `swift test --skip-build` on 22 July 2026.
...
- A packaged iPad app composition root, automatic manifest-date polling, and companion playback.
...
The next coding slice is **read-only iPad playback foundation**. Do not allow companion clients to alter the catalogue.
