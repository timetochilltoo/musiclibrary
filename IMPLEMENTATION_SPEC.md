...
- Device-local iPad favourites complete: `CompanionPreferenceStore` persists starred published album IDs alongside companion state. Album detail can add/remove a favourite, the album list displays stars and filters favourites, and snapshot refreshes do not alter those preferences. They never enter the snapshot or Mac catalogue.
