...
Implementation baseline (12 August 2026): the Mac catalogue, retained local/NAS authorization, safe scanner and review queue, explicit metadata comparison, digital assets, local playback and playlists, FLAC tag-write safety, manual lyrics, recovery/backup/export, snapshot publication, and the read-only companion foundation are implemented. The current package passes 86 automated tests. Live NAS endurance and iPad device validation remain deliberately deferred while the Mac artwork-first redesign is completed. See [HANDOFF.md](HANDOFF.md) for the current Git baseline and next implementation slice.
...
**Import-to-existing-edition workflow (12 August 2026):** an approved release proposal now offers two explicit outcomes: create a new edition, or attach its digital files to a selected existing edition. Before attachment, the Mac shows every imported-file-to-catalogue-track pairing. A populated edition must have an exact disc set and per-disc track count; an empty edition may receive the imported disc/track structure. Catalogue album/track titles, credits, artwork, and edition metadata are preserved, reused root-relative paths are rejected, and compatibility is revalidated inside the same atomic transaction. This closes the duplicate-album workflow gap without guessing identity or changing source files.
...
**Implementation checkpoint (13 August 2026):** slices 1, 2, and the compact-player portion of slice 4 are implemented. The Mac sidebar is grouped by Library, Organize, Review, and Settings; Albums now has artwork grid/list presentations, persistent source/favourite/sort controls, batched selected-front-artwork lookup, and an artwork-led detail hero with Play and Shuffle actions. The persistent player now shows title/source-format context, elapsed and total time, seek progress, transport, visible shuffle/repeat state, volume, and stop. Artwork decoding is performed away from the main actor. Browsing these surfaces does not change the catalogue. Library Changes, playlists, Settings, health presentation, accessibility polish, and real-window visual checks remain the active visual slices.
