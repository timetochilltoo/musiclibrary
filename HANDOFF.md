...
Add the approved child-folder scan action after validating the chosen folder descends from one registered root. Preserve the registered root-relative paths when scanning a child folder, keep identities/order stable, make every change explicit and transactional, and keep corrections catalogue-only. Add a later review action for safely removing obsolete catalogue references only after the user has reviewed affected albums/tracks. Do not begin artwork migration or snapshot-to-master reconstruction without a new reviewed design.
