...
Build scan reconciliation on top of storage-root scope: when an available folder is rescanned, compare known root-relative assets to the scan, present missing assets/albums as reviewable candidates, and require explicit confirmation before removing catalogue references. An offline NAS root must remain offline, never be interpreted as deleted. Add the approved child-folder scan action after validating the chosen folder descends from one registered root. Preserve stable identities and ordering, make each action explicit and transactional, and keep all corrections catalogue-only. Do not begin artwork migration or snapshot-to-master reconstruction without a new reviewed design.
