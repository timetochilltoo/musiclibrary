...
The local/NAS folder workflow now has its planned safe review and cleanup actions. The next product work should be driven by fresh user testing of full-root rescan, child-folder import, and the two missing-file choices. Do not begin artwork migration or snapshot-to-master reconstruction without a new reviewed design.
