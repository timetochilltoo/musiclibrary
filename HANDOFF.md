...
The local/NAS folder workflow, complete safe scanning workflow, explicit legacy-artwork migration, field-level catalogue activity history, and the first three audit-hardening slices are implemented. The next coding slice is **reliability/performance cleanup**: inspect remaining main-actor/file-I/O paths and repeated work, make only low-risk measurable improvements, and add focused regression coverage where needed. After each hardening slice, run the focused test and full validation appropriate to the change, update this handoff, and commit/push. Do not start the visual foundation or any artwork-first UI redesign until the hardening workstream exits. Automatic hash-based relinking, snapshot-to-master reconstruction, and WAV/DSF/other non-FLAC tag write-back remain deferred.
