...
The local/NAS folder workflow, complete safe scanning workflow, and explicit legacy-artwork migration are now implemented. The first user-facing check for this slice is to open an album containing an old path-only artwork record, choose **Make Portable**, confirm the managed label appears, and verify that the original image remains in place. After that, the next code slice should be a small field-level catalogue activity-history/diff surface (the existing history is revision/time only). Do not begin automatic hash-based relinking or snapshot-to-master reconstruction without a new reviewed design; WAV/DSF/other non-FLAC tag write-back also remains deferred.
