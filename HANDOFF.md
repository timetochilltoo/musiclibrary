...
`CompanionPreferenceStore` writes `CompanionPreferences.json` under iPad Application Support. It holds published album IDs starred by the user plus up to 20 recently played album IDs in newest-first order. The iPad album detail toggles favourites, album rows show a star, a **Favourites only** filter narrows local browsing, and starting playback of a different album's track records that album and exposes a **Recently played** list. The history respects the current search/favourites filter and has a local **Clear** action; clearing it leaves favourites unchanged. This data is intentionally outside the verified snapshot cache and Mac master: it is never published, uploaded, merged, or treated as catalogue data. Snapshot refresh preserves it; IDs that are absent from a newer snapshot simply do not appear until/if that album returns. It is not yet per-track history, a shared play count, or resume-position persistence.
