...
The local/NAS folder workflow, complete safe scanning workflow, explicit legacy-artwork migration, field-level catalogue activity history, and the first two audit-hardening slices are implemented. The next coding slice is **DSF/WAV validation**: make container arithmetic and header bounds overflow-safe, reject malformed/truncated files before allocation or playback, and add focused malformed-file tests. After each hardening slice, run the focused test and full validation appropriate to the change, update this handoff, and commit/push. Do not start the visual foundation or any artwork-first UI redesign until the hardening workstream exits. Automatic hash-based relinking, snapshot-to-master reconstruction, and WAV/DSF/other non-FLAC tag write-back remain deferred.
