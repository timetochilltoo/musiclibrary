...
The local/NAS folder workflow now has its planned safe review and cleanup actions plus verified manual replacement-file proposals. A full-root rescan also highlights only new unregistered paths, with all scanned files retained as diagnostics. The next product work should be driven by user testing of a new album-folder addition and its metadata proposal creation, as well as normal rescan behaviour after no changes. Do not begin artwork migration, automatic hash-based relinking, or snapshot-to-master reconstruction without a new reviewed design.
