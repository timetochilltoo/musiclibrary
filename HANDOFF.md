...
The local/NAS folder workflow, safe scanning/reconciliation workflow, new-versus-existing-edition import decision, legacy-artwork migration, field-level catalogue activity history, audit hardening, and the planned artwork-first Mac presentation workstream are implemented. Do not begin another broad feature or redesign pass by assumption. The next useful work is evidence-driven real-library validation: local and NAS playback latency/endurance, offline/reconnect behavior, large-library scrolling/search, VoiceOver/contrast inspection, and user feedback on the redesigned Mac surfaces. Automatic hash-based relinking, snapshot-to-master reconstruction, WAV/DSF/other non-FLAC tag write-back, internet lyrics providers, AI modules, live NAS endurance, and iPad device validation remain deferred; the provider/format choices in the Open Decisions section still require the user.
