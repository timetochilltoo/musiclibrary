...
### Existing-edition import attachment (12 August 2026)

The first post-audit workflow gap is implemented across `MusicDomain`, `MusicPersistence`, `MusicApplication`, and the Mac app. An approved Library Changes proposal now has distinct **Create New Edition** and **Attach to Existing Edition** actions. The attachment sheet searches active catalogue editions and displays an explicit imported-file-to-catalogue-track preview. Populated targets require an exact disc set and per-disc track count; empty targets can receive the imported structure. Catalogue titles, credits, artwork, and edition metadata are never overwritten. Root-relative asset paths are checked for reuse and every condition is revalidated inside the attachment transaction. Repeating the same attachment is idempotent; mismatch tests verify atomic rollback and no revision. `swift test` passes 86 tests in 8 suites. No user Application Support catalogue or source media was touched by development validation.

### Next safe slice

The local/NAS folder workflow, safe scanning/reconciliation workflow, new-versus-existing-edition import decision, legacy-artwork migration, field-level catalogue activity history, and audit hardening are implemented. The pre-UI audit gate is closed and the user has explicitly authorized the artwork-first Mac redesign. Continue with the visual foundation in `BUILD_PLAN.md`: grouped native sidebar, artwork grid/list albums with source/filter/sort controls, album-detail hero and progressively disclosed files/metadata, then the review/player/settings polish. Preserve every existing persistence and source-file safety invariant. Automatic hash-based relinking, snapshot-to-master reconstruction, WAV/DSF/other non-FLAC tag write-back, internet lyrics providers, AI modules, live NAS endurance, and iPad device validation remain deferred.
