...
The local/NAS folder workflow, complete safe scanning workflow, explicit legacy-artwork migration, field-level catalogue activity history, and the audit hardening slices are implemented. The pre-UI audit gate is closed. The next product workstream is the previously agreed artwork-first Mac UI redesign, but it has intentionally not started in this task; begin it only in a separate follow-up after rereading the current visual direction in `BUILD_PLAN.md`. Automatic hash-based relinking, snapshot-to-master reconstruction, and WAV/DSF/other non-FLAC tag write-back remain deferred. Real-world NAS playback endurance/latency and manual permission/artwork workflows remain user-validation items, not automated exit checks.
