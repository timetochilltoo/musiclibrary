...
The local/NAS folder workflow, complete safe scanning workflow, explicit legacy-artwork migration, field-level catalogue activity history, and the audit hardening slices are implemented. The next coding slice is **remaining local error/performance review**: reduce avoidable registered-root/status refresh work where it is demonstrably safe, and decide whether currently swallowed playback/UI errors need user-visible diagnostics without changing the intentional “skip unavailable item” playback behavior. After each hardening slice, run the focused test and full validation appropriate to the change, update this handoff, and commit/push. Do not start the visual foundation or any artwork-first UI redesign until the hardening workstream exits. Automatic hash-based relinking, snapshot-to-master reconstruction, and WAV/DSF/other non-FLAC tag write-back remain deferred.
