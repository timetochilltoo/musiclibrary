...
The local/NAS folder workflow now has its planned safe review and cleanup actions plus manual replacement-file proposals. The next user test is: full-root rescan with one safely moved file → **Choose Replacement File…** → select the replacement beneath the same root → Settings → review and apply the proposal → Library Health recheck confirms it is available. Do not begin artwork migration, automatic hash-based relinking, or snapshot-to-master reconstruction without a new reviewed design.
