...
The last verified baseline contains 44 tests in 5 suites, run with a rebuilt `swift test` followed by `swift test --skip-build` on 23 July 2026. Run `swift test`; do not rely on this handoff alone.
...
The initial MusicBrainz adapter is now available from each Import Inbox release proposal's **Search MusicBrainz…** button. It sends only the user-visible title and optional artist text after the user presses **Search MusicBrainz**, uses an identifiable User-Agent and a one-request-per-second request gate, and displays title/artist/date/country/catalogue-number/disc-count results in an ephemeral preview sheet. It does not upload audio, cache a result, change a proposal, alter catalogue data, or modify source files. The next extension must introduce a persisted, field-by-field comparison and explicit selective acceptance; do not add an automatic lookup or direct overwrite path.
...
Persist selected external-result proposals and implement a field-by-field, Mac-only acceptance preview. Acceptance must be explicit per field and must preserve provenance; it must never write source tags or run automatically. Do not begin artwork migration or snapshot-to-master reconstruction without a new reviewed design.
